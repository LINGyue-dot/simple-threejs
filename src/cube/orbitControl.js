...
import { Matrix4, Raycaster, Vector3, Vector4 } from "three";

export default class OrbitControl {
  // 当鼠标在 cubes 上时候不进行旋转
  constructor(dom, camera, cubes) {
    this.dom = dom;
    this.camera = camera;
    ...
    dom.addEventListener("mousedown", event => {
      raycast.setFromCamera({ x: event.clientX / window.innerWidth * 2 - 1,
        y: -event.clientY / window.innerHeight * 2 + 1 }, camera);
      const intersectObjects = raycast.intersectObjects(cubes);
    ...
    dom.addEventListener("mousemove", event => {
      ...
      // TODO 可以加个 scale 因子
      store.type === "left" && this.rotate.call(this, -deltaY / 400 * Math.PI, -deltaX / 400 * Math.PI);
